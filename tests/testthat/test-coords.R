test_that("grid_to_coords() works", {
  # 5km
  grid5km <- parse_grid("6542412", "5km")
  expect_equal(coords_to_grid(142.23824, 43.67456, "5km"), grid5km)
...
test_that("coords_reconversion", {
  ...
  for (grid_size in c(
    ...
    grid <- parse_grid(x, grid_size, strict = FALSE)
...
test_that("coords_reconversion_notcenter", {
  ...
  for (grid_size in c(
    ...
    grid <- parse_grid(x, grid_size = grid_size, strict = FALSE)
    coords <- grid_to_coords(grid, center = FALSE)
    grid1 <- coords_to_grid(
      coords$X_min + eps,
      coords$Y_min + eps,
      grid_size = grid_size
    )
    grid2 <- coords_to_grid(
      coords$X_max - eps,
      coords$Y_min + eps,
      grid_size = grid_size
    )
    grid3 <- coords_to_grid(
      coords$X_min + eps,
      coords$Y_max - eps,
      grid_size = grid_size
    )
    grid4 <- coords_to_grid(
      coords$X_max - eps,
      coords$Y_max - eps,
      grid_size = grid_size
    )
