...
geometry_to_grid <- function(
  ...
  if (inherits(geometry, "sfc_POINT")) {
    ...
    coords_to_grid(X = coords$X, Y = coords$Y, grid_size = grid_size)
  } else {
    geometry <- geometry |>
      sf::st_as_sf() |>
    ...
    geometry_grid <- geometry |>
      stars::st_rasterize(grid, options = options, ...) |>
      sf::st_as_sf() |>
      ...
      sf::st_drop_geometry() |>
      dplyr::mutate(
        grid = coords_to_grid(X = .data$X, Y = .data$Y, grid_size = grid_size)
      ) |>
      dplyr::distinct(.data$id, .data$grid) |>
      dplyr::summarise(dplyr::across("grid", list), .by = "id")

    geometry |>
      sf::st_drop_geometry() |>
      dplyr::left_join(geometry_grid, by = "id") |>
      dplyr::pull("grid")
  }
}
...
#' @export
st_as_sfc.grid <- function(x, as_points = FALSE, crs = sf::NA_crs_, ...) {
  geometry <- tibble::tibble(grid = x) |>
    vec_unique()
  geometry <- vec_slice(geometry, !is.na(geometry$grid))

  if (as_points) {
    geometry$geometry <- grid_to_coords(geometry$grid, center = TRUE) |>
      sf::st_as_sf(coords = c("X", "Y"), ...) |>
      sf::st_geometry()
  } else {
    coords <- grid_to_coords(geometry$grid, center = FALSE)
  ...
  tibble::tibble(grid = x) |>
    dplyr::left_join(geometry, by = "grid") |>
    purrr::chuck("geometry") |>
    sf::st_set_crs(crs)
}

#' @export
plot.grid <- function(x, y, as_points = FALSE, ...) {
  if (!missing(y)) {
    warn("`y` is ignored")
  }
...
grid_as_sf <- function(
  ...
  x |>
    sf::st_set_geometry(
      grid |>
        st_as_sfc(as_points = as_points, crs = crs)
    ) |>
    sf::st_as_sf(...)
}
